    ...
    try:
        ...
        if not expirations:
            ...
            if ticker.endswith(".MC") or any(ticker.endswith(f".{sfx}") for sfx in ["L", "PA", "AS", "MI", "DE", "SW", "HE", "ST", "CO"]):
                ...
                print(f" [SYNTHETIC] Generating Black-Scholes options for {ticker}...")
                try:
                    synthetic_result = generate_synthetic_options(
                        ticker, current_price, historical_volatility, stock_data
                    )
